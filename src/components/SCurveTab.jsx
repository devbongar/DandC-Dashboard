  ...
  const chartData = useMemo(() => {
    if (viewMode === 'monthly') {
      return tableRows.map(r => ({
        ...
        target:    r.target_pct    ?? null,
        actual:    r.actual_pct    ?? null,
        projected: r.projected_display ?? null,
      }))
    }

    // Quarterly: take the last non-null value within each quarter (values are already cumulative)
    const qMap = {}, qOrder = []
    for (const r of tableRows) {
      const q = toQuarter(r.period_date)
      if (!qMap[q]) { qMap[q] = { period: q, target: null, actual: null, projected: null }; qOrder.push(q) }
      if (r.target_pct      != null) qMap[q].target    = r.target_pct
      if (r.actual_pct      != null) qMap[q].actual    = r.actual_pct
      if (r.projected_display != null) qMap[q].projected = r.projected_display
    }
    return qOrder.map(q => qMap[q])
  }, [tableRows, viewMode])
